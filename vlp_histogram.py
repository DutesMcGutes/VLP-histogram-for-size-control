...
try:
    ...
    # Filter columns with valid numerical data
    valid_columns = [col for col in data.columns if data[col].dropna().count() > 0]

    if not valid_columns:
        print("Error: No valid data found in the provided file.")
        exit()

    # Normalize data for histograms and KDE
    fig, ax = plt.subplots(figsize=(10, 6))
    colors = plt.cm.viridis(np.linspace(0, 1, len(valid_columns)))  # Use the Viridis colormap for styling
    ...
    ax.set_title("Normalized Histograms and KDE for All Samples", fontsize=14, fontweight="bold", color="#4B4B4B")
    ...
    stats = pd.DataFrame({
        "Mean": [round(data[column].mean(), 2) for column in valid_columns],
        "Median": [round(data[column].median(), 2) for column in valid_columns],
        "Mode": [
            round(mode(data[column].dropna()), 2) if len(data[column].dropna()) > 0 else "N/A"
            for column in valid_columns
        ],
        "Minimum": [round(data[column].min(), 2) for column in valid_columns],
        "Maximum": [round(data[column].max(), 2) for column in valid_columns],
        "N (Count)": [data[column].count() for column in valid_columns],
        "Std Dev": [round(data[column].std(), 2) for column in valid_columns]
    }, index=valid_columns)
